...
class USBDevice extends Device {
   ...
   * @param {USBDeviceInfo} options.device
  ...
  set(options) {
    ...
    if (options.device != null) {
      assert(options.device instanceof USBDeviceInfo);
  ...
  static async requestDevice() {
    ...
    const filters = [USBDeviceInfo.getDeviceFilter()];
    let device;

    try {
      device = await busb.usb.requestDevice({
        filters
      });
    } catch (e) {
    ...
    return USBDeviceInfo.fromUSBDevice(device);
  }
}

/**
 * Ledger USB Device info
 * @extends {DeviceInfo}
 */
...
class USBDeviceInfo extends DeviceInfo {
  ...
  set(options) {
    ...
    assert(options.device instanceof busb.USBDevice);
