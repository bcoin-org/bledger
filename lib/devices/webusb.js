...
class WebUSBDevice extends Device {
  ...
  enforce(value, reason) {
    if (!value)
      throw new DeviceError(reason, WebUSBDevice);
  }
  ...
  async open() {
    await WebUSBDevice.enforceSupport();
    ...
    try {
      ...
      await device.close();
      throw new DeviceError('Web Interface not available.');
    }
  ...
  static async isSupported() {
    return global.navigator && global.USB &&
      global.navigator.usb instanceof global.USB;
  }

  /**
   * Ensure WebUSB support.
   * @throws {DeviceError}
   */

  static async ensureSupport() {
    if (!this.isSupported())
      throw new DeviceError('WebUSB is not supported.', WebUSBDevice);
  }
