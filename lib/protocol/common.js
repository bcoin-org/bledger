...
common.BTC_MAGIC = '\x18Bitcoin Signed Message:\n';
...
common.encodeMessage = function encodeMessage(message, magic) {
  assert(message.length < 0xfff, 'Message is too big.');
  const messageLength = message.length < 0xfd ? 1 : 3;
  const bw = bufio.write(magic.length + messageLength + message.length);
